...
# 搜索测试用例
suit = unittest.defaultTestLoader.discover(Test_case_path,pattern='Smokecase.py')
print(suit)
# 执行测试用例+写报告
f = open(Test_report_name,'wb') #创建文档并且以二进制写入
HTMLTestRunner(stream=f,title='Smokecase自动化测试报告').run(suit)
f.close()#关闭文档

# 发邮件
mail = SendMail()
mail.Add_text('Smokecase自动化测试报告',Subject,From,To)
